...
describe('OrgMembershipService', () => {
    ...
    describe('verifyRole', () => {
        ...
        it('should return membership after verifying role', async () => {
            ...
            expect(mockPrisma.orgMembership.findUnique).toHaveBeenCalledWith({
                where: {
                    userId_orgId: { userId: 'some-user-id', orgId: 'some-org-id' }
                }
            })
        });
    ...
    describe('addMember', () => {
        ...
        it('should return membership', async () => {
            ...
            // Act
            const result = await service.addMember('some-org-id', 'some-user-id', 'OWNER');

            // Assert
            expect(result).toBe(mockMembership);
            expect(mockUserService.findById).toHaveBeenCalledWith('some-user-id');
        });
        ...
        it('should return NotFoundException if user is not found', async () => {
            ...
            // Act and Assert
            await expect(service.addMember('some-org-id', 'some-user-id', 'OWNER')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should return ConflictException is user is already a member', async () => {
            ...
            // Act and Assert
            await expect(service.addMember('some-org-id', 'some-user-id', 'OWNER')).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('removeMember', () => {
        ...
        it('should return membership', async () => {
            // Arrange
            mockPrisma.orgMembership.findUnique.mockResolvedValue({ role: 'MEMBER' });
            mockPrisma.orgMembership.delete.mockResolvedValue(mockMembership);

            // Act
            const result = await service.removeMember('some-org-id', 'some-user-id');
        ...
        it('should return ForbiddenException if user is owner. Cannot remove owner.', async () => {
            // Arrange
            mockPrisma.orgMembership.findUnique.mockResolvedValue({ role: 'OWNER' });

            // Act and Assert
            await expect(service.removeMember('some-org-id', 'some-user-id')).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should return NotFoundException if already deleted', async () => {
            ...
            // Act and Assert
            await expect(service.removeMember('some-org-id', 'some-user-id')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('updateRole', () => {
        ...
        it('should return membership', async () => {
            ...
            const result = await service.updateRole('some-org-id', 'some-user-id', 'ADMIN');
    ...
    describe('listMembers', () => {
        ...
        it('should return membership', async () => {
            mockPrisma.orgMembership.findMany.mockResolvedValue([{ ...mockMembership, ...mockUser }]);

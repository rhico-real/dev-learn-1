...
describe('RolesGuard', () => {
  ...
  const createMockContext = (userRole?: SystemRole, requiredRoles?: SystemRole[]): ExecutionContext => {
  ...
  it('should return true if user role matches required role', async () => {
    const context = createMockContext(SystemRole.SUPER_ADMIN, [SystemRole.SUPER_ADMIN]);
    const result = await guard.canActivate(context);
    expect(result).toBe(true);
  });

  it('should return false if user role does NOT match required role', async () => {
    const context = createMockContext(SystemRole.USER, [SystemRole.SUPER_ADMIN]);
    const result = await guard.canActivate(context);
  ...
  it('should return true if user is SUPER_ADMIN (the "God Mode" bypass)', async () => {
    ...
    const context = createMockContext(SystemRole.SUPER_ADMIN, [SystemRole.USER]);
    const result = await guard.canActivate(context);
    expect(result).toBe(true);
  });
});